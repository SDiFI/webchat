...
const InfoButton = styled.button<InfoButtonProps>`
    ...
    &:hover {
        background-color: ${props =>
            props.secondary ? props.theme.secondaryButtonHoverBg : props.theme.primaryButtonHoverBg};
        cursor: pointer;
    }
            ...
            <ButtonGroup>
                {buttons.map(btn => (
                    <InfoButton key={btn.text} onClick={typeof btn.action !== 'string' ? btn.action : undefined}>
                        {'string' === typeof btn.action ? (
                            <a href={btn.action} target='_blank' rel='noreferrer'>
                                {btn.text}
                            </a>
                        ) : (
                            btn.text
